...
@section('content')
                                        ...
                                        <tr>
                                            <td>
                                                <h2 class="table-avatar">
                                                    <a href="{{ url('employee/profile/'.$items->user_id) }}" class="avatar"><img alt="" src="{{ URL::to('/assets/images/employeePassport/'. $items->avatar) }}" alt="{{ $items->name }}"></a>
                                                    <a href="#">{{ $items->name }}<span>{{ $items->position }}</span></a>
                                                </h2>
                                            </td>
                                            <td hidden class="id">{{ $items->id }}</td>
                                            <td class="leave_type">{{$items->leavetype}}</td>
                                            <td hidden class="leave_from">{{ $items->leavefrom }}</td>
                                            <td>{{date('d F, Y',strtotime($items->leavefrom)) }}</td>
                                            <td hidden class="leave_to">{{$items->leaveto}}</td>
                                            <td>{{date('d F, Y',strtotime($items->leaveto)) }}</td>
                                            {{-- <td class="day">{{$items->day}} Day</td> --}}
                                            <td class="leave_reason">{{$items->leavereason}}</td>
                                            <td class="text-center">
                                                <div class="dropdown action-label">
                                                    
                                                        <a cclass="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">
                                                            @if ($items->status == NULL)
                                                            <i class="fa fa-dot-circle-o text-secondary"></i> Awaiting Supervisor Approval
                                                            @elseif ($items->status == 'supervisorApproved')
                                                            <i class="fa fa-dot-circle-o text-waring"></i> Awaiting HR Approval
                                                            @elseif ($items->status == 'hrApproved')
                                                            <i class="fa fa-dot-circle-o text-purple"></i> Awaiting DG Approval
                                                             @elseif ($items->status == 'dgApproved')
                        ...
                        @csrf
                            ...
                            <div class="form-group">
                                ...
                                <select class="select" name="employeeId">
                                    ...
                                    @else
                                    <option value="Null">No employees</option>
                                    @endif
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('form/leaves/edit') }}" method="POST">
                            ...
                            <div class="form-group">
                                <label>Leave Type <span class="text-danger">*</span></label>
                                <select class="select" id="e_leave_type" name="leavetype">
                            ...
                            <div class="form-group">
                                <label>From <span class="text-danger">*</span></label>
                                <div class="cal-icon">
                                    <input type="text" class="form-control datetimepicker" id="e_from_date" name="leavefrom" value="">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>To <span class="text-danger">*</span></label>
                                <div class="cal-icon">
                                    <input type="text" class="form-control datetimepicker" id="e_to_date" name="leaveto" value="">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Number of days <span class="text-danger">*</span></label>
                            ...
                            <div class="form-group">
                                <label>Leave Reason <span class="text-danger">*</span></label>
                                <textarea rows="4" class="form-control" id="e_leave_reason" name="leavereason" value=""></textarea>
                            </div>
                            <div class="submit-section">
    ...
    <script>
        $(document).on('click','.leaveUpdate',function()
        {
            ...
            $('#e_from_date').val(_this.find('.leavefrom').text());  
            $('#e_to_date').val(_this.find('.leaveto').text());  
            $('#e_leave_reason').val(_this.find('.leavereason').text());

            var leavetype = (_this.find(".leavetype").text());
            var _option = '<option selected value="' + leavetype+ '">' + _this.find('.leavetype').text() + '</option>'
