                                ...
                                <tr>
                                    <td>
                                        <h2 class="table-avatar">
                                            <a href="{{ url('employee/profile/'.$items->employeeId) }}" class="avatar"><img   style ="
                                                position: relative;
                                                ...
                                                overflow: hidden;
                                                "  src="{{ URL::to('/assets/images/employeePassport/'. $items->avatar) }}"></a>
                                            <a href="{{ url('employee/profile/'.$items->employeeId) }}">{{ $items->name }}<span>{{ $items->position }}</span></a>
                                        </h2>
                                    </td>
                                    <td>{{ $items->employeeId }}</td>
                                    <td>{{ $items->verifiedprofile }}</td>
                                    <td>{{ $items->email }}</td>
                                    <td>{{ $items->phone_number }}</td>
                                    <td>{{ $items->join_date }}</td>
                                    <td>{{ $items->role_name }}</td>
                                    <td class="text-right">
                                        @if($items->verifiedprofile != "Deleted")
                                        <div class="dropdown dropdown-action">
                                            <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a class="dropdown-item" href="{{ url('all/employee/view/edit/'.$items->employeeId) }}"><i class="fa fa-pencil m-r-5"></i> Edit</a>

                                                @if (Auth::user()->role_name == "Super Admin")
                                                <a class="dropdown-item" href="{{url('all/employee/delete/'.$items->employeeId)}}"onclick="return confirm('Are you sure to want to delete it?')"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                                <a class="dropdown-item" href="{{url('all/employee/viewchanges/'.$items->employeeId)}}" ><i class="fa fa-trash-o m-r-5"></i> View Changes</a>
                                                @endif
                                            </div>
                                        </div>
                                        @endif
